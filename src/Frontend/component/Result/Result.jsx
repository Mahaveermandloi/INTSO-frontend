  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setLevel(formData.level); // Ensure level is correctly set from formData

        // Extract student_status from the first item in the data array if it exists
        if (data.data.length > 0) {
          setStudentStatus(data.data[0].student_status);
        } else {
          setStudentStatus("");
        }
