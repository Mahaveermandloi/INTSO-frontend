      ...
      try {
        ...
        if (accessToken) {
          const response = await axios.get(
            `${URLPath}/api/v1/reward/getReward-data`,
            {
              headers: {
    ...
    if (selectedFile) {
      ...
      try {
        ...
        const response = await axios.post(
          `${URLPath}/api/v1/reward/post-reward`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
        ...
        if (response.status === 201) {
          ...
          toast.success("Reawrd uploaded successfully!", {
            position: "top-center",
            autoClose: 3000,
            hideProgressBar: false,
            closeOnClick: true,
          ...
          const newImage = {
            ...
            image: response.data.data.image, // Ensure this matches the image path in the response
            caption: response.data.data.caption, // Ensure this includes the caption if necessary
          };

          setReward((prevGallery) => [...prevGallery, newImage]);
    ...
    if (isConfirmed) {
      ...
      try {
        ...
        const response = await axios.delete(
          `${URLPath}/api/v1/reward/delete-reawrd-data/${id}`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
